...
def sample_transition(batch_size=16):
    ...
    return (
        tf.convert_to_tensor(sampled_current_states, dtype=tf.float32),
        tf.convert_to_tensor(sampled_actions, dtype=tf.int32),
        tf.convert_to_tensor(sampled_rewards, dtype=tf.float32),
        tf.convert_to_tensor(sampled_next_states, dtype=tf.float32),
        tf.convert_to_tensor(sampled_terminals, dtype=tf.bool),
    )
...
for episode in range(NUM_EPISODES):
    ...
    pd.DataFrame(metric).to_csv("lunar_lander.csv", index=False)
env.close()
q_net.save("dqn_lunar_q_net.keras")